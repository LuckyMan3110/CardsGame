...
const CardFive = () => {
    ...
    const OpenGameRoom = async (e) => {
        e.preventDefault();
        user.access_type == "user" ? navigate(`/joingame?gameType=${"Play CAH with your kids"}`) : navigate(`/opengame?gameType=${"Play CAH with your kids"}`);
    }
    useEffect(() => {
        if (isError) {
            navigate('/signin');
        ...
        <div className='rounded-sm border border-stroke bg-white py-6 px-7.5 shadow-default dark:border-strokedark dark:bg-boxdark' onClick={OpenGameRoom}>
            ...
            <div className='mt-4 flex items-end justify-between'>
                <div>
                    ...
                    <span className='text-sm font-medium'>Play CAH with your kids</span>
                </div>
            </div>
        </div>
    )
